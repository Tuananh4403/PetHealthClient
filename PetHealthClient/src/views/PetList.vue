      ...
      <tbody>
        <tr v-for="(pet, index) in pets" :key="index">
          <td v-for="header in headers" :key="header">
            <template v-if="header !== 'Detail' && header!=='gender'">
              {{ pet[header] || '' }}
            </template>
            <template v-else-if="header === 'gender'">
                {{ (pet[header] ? 'Male' : "Female")}}
              </template>
              <template v-else>
              <button class="update-btn" @click="navigateTo('medical-detail/'+ pet['id'] )">Detail</button>
            </template>
          </td>
        </tr>
      </tbody>
...
import { toastSuccess, toastWarning } from '@/utils/toast';
  ...
  export default {
    ...
    data() {
      return {
      headers: ['petName', 'kindOfPet', 'species', 'gender', 'birthday','customerName', 'Detail'],
    ...
    methods: {
      ...
      fetchRecord() {
        axiosPrivate.get("/api/pet/get-list-pet")
          .then(response => {
